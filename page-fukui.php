<?php
/*
Template Name: home-fukui
*/
?>
<?php get_header("fukui"); ?>
...
<main id="top" class="content">
  ...
  <section id="lead">
    <div class="lead_inr">
      ...
      <p>この創業パックは企画・制作段階での徹底的な効率化によって、<?php echo $city_fukui; ?>トップクラスのクオリティを保ちながら、最安値プランでは <span>49,800円</span>という<br><?php echo $city_fukui; ?>のホームページ制作会社の中では圧倒的低価格でホームページを制作することが可能になりました。<br>また本気でWEBからの集客を考えられているお客様からは、しっかりとしたSEO対策が可能なゴールドプラン以上も大変好評です。</p>
  ...
  <section id="seo">
    <div class="seo_inr">
      ...
      <div class="lead">
        <i><img src="<?php echo get_template_directory_uri(); ?>/images/seo_icon.png" alt=""></i>
          <div><h3><?php echo $city_fukui; ?>ホームページ制作.COMのSEO対策サービスは内部対策中心で、<br>他者の外部リンクに頼ったSEO対策は行いません。</h3>
    ...
    <div class="lead">
      ...
      <div>
        <h3><?php echo $city_fukui; ?>だけでなく<?php echo $place_fukui; ?>県全域のSEO対策を承ります。</h3>
        <p>弊社の事業拠点である<?php echo $city_fukui; ?>市はもちろん、<?php echo $place_fukui; ?>県全域・石川県全域・富山県全域といった北陸におけるホームページの最適化を全力で支援いたします。これまでのデザインベースのホームページ制作だけでは結果に繋がりにくく、コンテンツSEOとデザインを両立したホームページ集客支援を弊社では心がけております。SEO対策やホームページ制作といったWEB全般のお悩みはお気軽にご相談ください！</p>
      </div>
    </div>
    <div class="method">
      <ul>
        <li><h5>戦略的SEO</h5><p>SEOはお客様のターゲットをしっかりと把握して、WEB市場の調査、キーワード調査を行いビッグキーワードをはじめとしたミドルキーワードといった成果の上がりやすいものから施策展開を行い競合他社に勝るSEO戦略を立案いたします。<?php echo $city_fukui; ?>ホームページ制作.COMでは、内部・外部SEOにおける自社チェック項目を活用しておりますので「もれなく・だぶりなく」SEO対策を行い健全なホワイトハットを心がけておりますのでご安心ください。</p></li>
        ...
        <li><h5>臨機応変</h5><p>SEOにおいて重要な要素である検索エンジンのアルゴリズムですが、これらは日々ルールの変更を行なっております。例えば、スマートフォンが普及した昨今では、スマホ対応（モバイルフレンドリー）がされていないサイトでは、検索結果が著しく下がるといった環境にあります。このような検索エンジンのルールが日々変更されている中で、<?php echo $city_fukui; ?>ホームページ制作.COMでは検索エンジンのルールが変わるたびにホームページを適応させ検索順位の下降を防ぎます。</p></li>
        <li><h5>結果重視</h5><p>ホームページ集客のゴールは、アクセスを増やすことではなくあくまで事業の売上に貢献することです。SEOは検索順位が上位に表示され、トラフィックを増やすことが目的ではありません。本当の目的は反響や売上が増えて初めて目標の達成と言えるのではないでしょうか。<?php echo $city_fukui; ?>ホームページ制作.COMでは「結果にコミットするホームページ」の提案（新規ホームページ制作からリニューアル）まで一貫してご提案いたします。</p></li>
      </ul>
      <div class="view_btn"><a class="arrow" href="#contact">相談してみる</a></div>
    </div>
    </div>
        ...
        <ul class="col2">
          <li>
           <h4>代理店に頼らない独自集客をするホームページ制作会社です</h4>
            <p><?php echo $city_fukui; ?>にある通常のホームページ制作会社では、代理店経由で仕事を受注するため少ない⼈材とコストで業務を⾏わなければならないといったホームページ制作業界独特の風潮があります。</p>
